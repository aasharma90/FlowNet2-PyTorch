...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  fprintf('Laser Timestamps read from %s\n', [laser_dir '../' laser '.timestamps']);
  ...
  fprintf('Start Timestamp Initial = %s and Laser Timestamps(1,1) = %s\n', num2str(start_timestamp), num2str(laser_timestamps(1,1)));
  fprintf('End   Timestamp Initial = %s and Laser Timestamp(end,1) = %s\n', num2str(end_timestamp), num2str(laser_timestamps(end,1)));
  start_timestamp = max(start_timestamp, laser_timestamps(1,1));
  end_timestamp = min(end_timestamp, laser_timestamps(end,1));
  fprintf('Start Timestamp Final = %s\n', num2str(start_timestamp));
  fprintf('End   Timestamp Final = %s\n', num2str(end_timestamp));
  
  start_index = find(laser_timestamps(:,1) >= start_timestamp, 1, 'first');
  end_index = find(laser_timestamps(:,1) <= end_timestamp, 1, 'last');
  fprintf('Start Index = %d, laser_timestamps(Start Index) = %s\n', start_index, num2str(laser_timestamps(start_index)));
  fprintf('End   Index = %d, laser_timestamps(Start Index) = %s\n', end_index  , num2str(laser_timestamps(end_index)));
